...
def recognize_structure(img):
    ...
    cv2_imshow(img_bin, "Binary Image")
    # inverting the image
    img_bin = 255 - img_bin
    cv2_imshow(img_bin, "Reversed Image")
    ...
    for c in contours:
        ...
        if (w < 0.9 * img_width and h < 0.9 * img_height):
            ...
            box.append([x, y, w, h])
            cv2_imshow(image, "Result Image{}".format(idx))
            idx += 1
...
finalboxes, output_img = recognize_structure(original_img)
