...
class TSR:
    ...
    def image_processing(self) -> None:
        ...
        prev_thinning_image = None
        thinning_image = ~binary_image
        thinning_cnt = 0
        while True:
            thinning_image = cv2.erode(thinning_image, thin_kernel, iterations=1)
            thinning_cnt += 1
            if thinning_image.sum() == 0:
                if prev_thinning_image is None:
                    thinning_image = ~binary_image
                else:
                    thinning_image = prev_thinning_image
                break
            else:
                prev_thinning_image = thinning_image
